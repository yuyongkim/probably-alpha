...
# --------------------------------------------------------------------------- #
# Broker reports (Google Drive primary corpus)                                #
# --------------------------------------------------------------------------- #
...
def broker_search(
    ...
    try:
        ...
        return _envelope(None, error={"code": "BROKER_SEARCH_FAILED", "message": str(exc)}, ok=False)
    meta = r.meta() or {}
    return _envelope(
        {
            "query": q,
...
def broker_vector_search(
    ...
    try:
        ...
        return _envelope(None, error={"code": "BROKER_VECTOR_SEARCH_FAILED", "message": str(exc)}, ok=False)
    meta = r.meta() or {}
    return _envelope(
        {
            "query": q,
